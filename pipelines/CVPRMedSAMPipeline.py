...
class CVPRMedSAMPipeline:
    ...
    def __init__(self, target_length, bbox_shift):
        self.target_length, self.bbox_shift = target_length, bbox_shift
    ...
    def preprocess_2D(self, img, gt, meta, target_length, bbox_shift):
        img_padded, img_resize_shape = self.img_transform(img, target_length)
        gt2D = self.target_transform(gt, meta['npz_path'], target_length, img_resize_shape)
    ...
    def _resize_longest_side(self, image, target_length):
        """
    ...
    def target_transform(self, gt, npz_path, target_length, img_resize_shape):
        gt_resize = cv2.resize(
            gt,
            (img_resize_shape[1], img_resize_shape[0]),
