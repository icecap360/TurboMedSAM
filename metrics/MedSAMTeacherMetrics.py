...
class MedSAMTeacherMetrics(BaseMetric):
    ...
    def get_metrics(self, pred, target, device) -> dict:
        ...
        preds = (torch.sigmoid(student_outputs)>0.5).type(torch.uint8)
        return {
            'normalized_surface_density': torch.mean(self.NSD_metric(preds, target_mask).reshape(-1)),
            'dice': torch.mean(self.dice_metric(preds, target_mask).reshape(-1)),
        }
